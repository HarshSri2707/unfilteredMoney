...
// ─────────────────────────────────────────────────────────
//  TopPicks
//  SPACING SYSTEM (same across ALL sections):
//   Section padding  →  py-10 md:py-14
//   Heading block    →  mb-6 md:mb-8
//   Cards/grid gap   →  gap-6
//   Max width        →  max-w-[1200px]
//  + Glassmorphism rating badge (dark, blur, bottom-right)
//  + Mobile image height h-52 (not squished)
// ─────────────────────────────────────────────────────────
...
const TopPicks = ({ picks = [], type, onProductClick, isFiltered = false }) => {
      ...
      {!isFiltered && (
        ...
        >
          <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
            ...
            {/* ── HEADING BLOCK ── mb-6 md:mb-8 uniform */}
            <motion.div
            ...
            >
              ...
              {/* Subtitle — mt-2 uniform gap from heading */}
              <p
                className="mt-2 text-sm md:text-base font-semibold text-neutral-500"
                style={{ fontFamily: 'Lato, sans-serif' }}
              >
                {type === 'credit-cards'
                  ? 'Cards we actually recommend this month'
                  : type === 'insurance'
                  ? 'Policies with best claim settlement ratios'
                  : 'Funds with proven long-term track records'}
              </p>
            </motion.div>

            {/* ── CARDS GRID ── gap-6 uniform */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {picks.slice(0, 3).map((pick, idx) => (
                <motion.div
                  key={pick.id}
  ...
  <div
    ...
    style={{
      background: 'rgba(0, 0, 0, 0.5)', // Image jaisa dark glass effect
      backdropFilter: 'blur(8px)',     // Blur thoda badhaya hai image look ke liye
  ...
  >
    ...
    <svg 
      ...
      height="12" 
      viewBox="0 0 24 24" 
      fill="#FBBF24" // Tailwind Amber-400 (Star color)
    ...
    {/* Rating Value */}
    <span 
      className="text-white text-[13px] font-bold leading-none mb-[0.5px]" 
      style={{ fontFamily: 'Lato, sans-serif' }}
    >
      {pick.rating}
    </span>
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      {/* Benefits */}
                      <div className="mb-4 flex-1">
                        <p className="text-[13px] font-bold text-neutral-600 mb-1" style={{ fontFamily: 'Lato, sans-serif' }}>
                          Benefits & Features
                        </p>
